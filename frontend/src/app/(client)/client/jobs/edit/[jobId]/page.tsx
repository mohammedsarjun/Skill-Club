...
function CreateJobPost() {
    ...
    async function fetchJobData() {
      try {
              ...
              if (jobResponse.success) {
                const jobResponseData:JobDetailResponseDTO=jobResponse.data;
                const jobData: JobData = {
                  title: jobResponseData.jobTitle,
                  description: jobResponseData.jobDescription,
                  category: jobResponseData.category.categoryId,
                  specialities: jobResponseData.specialities.map((s) => s.specialityId),
                  skills: jobResponseData.skills.map((s) => s.skillId),
                  rateType: jobResponseData.budget.rateType,
                  hourlyRate: jobResponseData.budget.rateType === "hourly"
                    ? {
                        min: jobResponseData.budget.min as number,
                        max: jobResponseData.budget.max as number,
                        hoursPerWeek: jobResponseData.budget.hoursPerWeek as number,
                        estimatedDuration: jobResponseData.budget.estimatedDuration as "1 To 3 Months" | "3 To 6 Months",
                      }
                    : undefined,
                  fixedRate: jobResponseData.budget.rateType === "fixed"
                    ? {
                        min: jobResponseData.budget.min as number,
                        max: jobResponseData.budget.max as number,
                      }
                ...
                setJobSavedData(jobData);
                sessionStorage.setItem("jobSavedData", JSON.stringify(jobData));
              }
            } catch (err) {
  ...
  async function handleClickNext() {
    ...
    if (step != totalSteps) {
      ...
      const jobCreateResponse = await clientActionApi.updateJobDetail(jobIdParam as string, parsedJobData);
      sessionStorage.removeItem("jobSavedData");
      if (jobCreateResponse.success) {
        toast.success(jobCreateResponse.message);
        route.push(`/client/jobs/${jobIdParam}`);
      } else {
