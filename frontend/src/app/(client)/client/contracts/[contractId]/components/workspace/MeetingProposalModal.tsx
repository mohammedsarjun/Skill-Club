...
import { Video, X, Calendar, Clock, CheckCircle } from 'lucide-react'
import { MeetingProposalModalProps } from '@/types/interfaces/IMeetingProposal'
import { meetingLinkSchema } from '@/utils/validations/validation'
import { z } from 'zod'
...
const MeetingProposalModal = ({ isOpen, onClose, onSubmit, contractId }: MeetingProposalModalProps) => {
  ...
  const handleSubmit = () => {
    const parsed = meetingLinkSchema.safeParse(meetingLink);
    if (!parsed.success) {
      setMeetingLinkError(parsed.error.issues[0].message);
      return;
    }
        ...
        <div className="p-8">
          ...
          <div className="mb-8">
            <div className="flex items-center gap-3 mb-4">
              ...
              <h3 className="text-xl font-semibold text-gray-800">Meeting Link <span className="text-sm text-gray-500 font-normal">(Optional)</span></h3>
            </div>
            <div>
              <input
                type="url"
                ...
                onChange={(e) => {
                  ...
                  if (meetingLinkError) setMeetingLinkError(null);
                }}
                placeholder="https://meet.google.com/xxx-xxxx-xxx or https://zoom.us/j/xxxxx"
                className={`w-full px-5 py-4 rounded-xl focus:outline-none transition-all duration-200 text-gray-800 placeholder:text-gray-400 ${meetingLinkError ? 'border-red-400 ring-1 ring-red-200' : 'border-2 border-gray-200 focus:border-indigo-600'}`}
              />
              {meetingLinkError && (
                <p className="mt-2 text-sm text-red-600">{meetingLinkError}</p>
              )}
            </div>
          </div>
                ...
                <div className="space-y-2">
                  ...
                  {meetingLink && (
                    <div className="flex items-center gap-2 mt-3 pt-3 border-t border-indigo-200">
                      ...
                      <p className="text-sm text-gray-700 truncate">{meetingLink}</p>
                    </div>
                  )}
                </div>
              </div>
