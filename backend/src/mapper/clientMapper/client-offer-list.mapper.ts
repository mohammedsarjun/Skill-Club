...
export const mapOfferModelToClientOfferListItemDTO = (offer: IOffer): ClientOfferListItemDTO => {
  const rawId = docIdToString((offer as unknown as Record<string, unknown>)._id) || docIdToString((offer as unknown as Record<string, unknown>).id);
  const freelancerPop = (offer.freelancerId as unknown) as Partial<{ firstName?: string; lastName?: string; freelancerProfile?: { logo?: string }; toString?: () => string }>;
  const freelancer = typeof offer.freelancerId === 'object' && offer.freelancerId !== null
    ? {
